...
# ============================
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/upsubbotgroup') # Group link for the bot
OWNER_LNK = environ.get('OWNER_LNK', 'https://t.me/SadeshaHansana2') # Owner link for the bot
...
IMDB = is_enabled((environ.get('IMDB', "Galse")), False)    # IMDB Results On (True) / Off (False)
...
# Allowed file extensions for subtitle bot (only these will be indexed)
...
# Send all button visibility for non-premium (False = hide, True = show with alert)
SHOW_SENDALL_NON_PREMIUM = is_enabled(os.environ.get('SHOW_SENDALL_NON_PREMIUM', 'True'), True)  # Show button but give alert

# Default caption format for subtitle files (can be overridden by group settings)
SUBTITLE_CAPTION = environ.get('SUBTITLE_CAPTION', 
    "{file_name}\n\n📁 File Size : {file_size}\n\n⚜️ Powered By : [ Ultra Pro Bots ]\n⚜️ Movie Download Bot : [ Ultra Pro Filter Bot ]")

# Forward protection: if True, non-premium users cannot forward files
ENABLE_FORWARD_PROTECTION = is_enabled(os.environ.get('ENABLE_FORWARD_PROTECTION', 'True'), True)
