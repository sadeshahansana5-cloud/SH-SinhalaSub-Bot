...
from info import ADMINS, BROADCAST_KEEP_BUTTONS
...
async def broadcast_users(bot, message):
    ...
    async def send(user):
        try:
            # Use users_broadcast which already copies the message (including buttons)
    ...
    async with lock:
        async for chat in chats:
            ...
            if temp.B_GROUPS_CANCEL:
                ...
                break
            try:
                # groups_broadcast already copies message with buttons
...
# ==============================================================================
# NEW ADDITIONS FOR SINHALA SUBTITLE BOT (NO DELETIONS)
# ==============================================================================
# Note: The broadcast functions already keep original message buttons because
# they use message.copy() which preserves all markup and buttons.
# No changes needed beyond the existing code. The feature BROADCAST_KEEP_BUTTONS
# is already satisfied because users_broadcast and groups_broadcast in utils.py
# use message.copy(chat_id=user_id). So no extra code needed here.
