...
from info import CHANNELS, MOVIE_UPDATE_CHANNEL, LINK_PREVIEW, ABOVE_PREVIEW, BAD_WORDS, LANDSCAPE_POSTER, TMDB_POSTER, SUBTITLE_ONLY_MODE, ALLOWED_EXTENSIONS, ENABLE_REQUESTS
...
async def media_handler(bot, message):
    ...
    # Subtitle-only mode: only accept documents with allowed extensions
    if SUBTITLE_ONLY_MODE:
        # If it's not a document, skip
        if not hasattr(message, "document"):
            logger.info(f"Skipping non-document media in subtitle-only mode: {media.file_name}")
            return
        # Check extension
        ext = os.path.splitext(media.file_name)[1].lower()
        if ext not in ALLOWED_EXTENSIONS:
            logger.info(f"Skipping file with disallowed extension: {media.file_name}")
            return
    ...
    # If request system is enabled, after saving a subtitle file, mark related requests as fulfilled
    if ENABLE_REQUESTS and success:
        # Extract base name from filename (simple version, can be improved)
        base = os.path.splitext(media.file_name)[0]
        # Mark requests containing this base name as fulfilled (case-insensitive)
        await db.mark_request_fulfilled(base)

    try:
        # Only handle movie updates if not in subtitle-only mode, or if user wants updates for subtitles
        if not SUBTITLE_ONLY_MODE and await db.movie_update_status(bot.me.id):
