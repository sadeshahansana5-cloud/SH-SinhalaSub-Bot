...
async def save_file(media):
    ...
    # Subtitle-only mode: skip if file extension not allowed
    if SUBTITLE_ONLY_MODE:
        ext = os.path.splitext(file_name)[1].lower()
        if ext not in ALLOWED_EXTENSIONS:
            logger.info(f"Skipping {file_name} - extension not allowed")
            return False, 4  # New code for skipped
...
async def get_search_results(chat_id, query, file_type=None, max_results=None, offset=0, filter=False):
    ...
    elif SUBTITLE_ONLY_MODE:
        # In subtitle-only mode, we only want documents (files), not videos/audio
        filter_mongo["file_type"] = "document"
