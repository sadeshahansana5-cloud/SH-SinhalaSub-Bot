...
class Database:    
    def __init__(self, uri, database_name):
        ...
        # New collection for subtitle requests
        self.subtitle_requests = self.db.subtitle_requests
    ...
    # ==============================================================================
    # NEW METHODS FOR SUBTITLE REQUESTS
    # ==============================================================================
    async def add_request(self, user_id, movie_name, year, title):
        """Add a subtitle request to database."""
        request = {
            "user_id": user_id,
        ...
        await self.subtitle_requests.insert_one(request)

    async def get_requests(self, limit=50):
        """Get recent subtitle requests."""
        cursor = self.subtitle_requests.find().sort("timestamp", -1).limit(limit)
        return await cursor.to_list(length=limit)

    async def mark_request_fulfilled(self, movie_name):
        """Mark requests for a movie as fulfilled (when subtitles added)."""
        await self.subtitle_requests.update_many(
            {"movie_name": {"$regex": movie_name, "$options": "i"}, "status": "pending"},
