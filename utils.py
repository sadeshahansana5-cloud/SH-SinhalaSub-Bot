...
# ==============================================================================
# NEW FUNCTIONS FOR SINHALA SUBTITLE BOT (ADDED WITHOUT DELETING ANYTHING)
# ==============================================================================

async def check_group_allowed(chat_id):
    """Check if the group is allowed to use the bot (if ALLOWED_GROUP_IDS is set)."""
...
async def can_forward_file(user_id):
    """Check if user is allowed to forward files (premium users can)."""
    if not ENABLE_FORWARD_PROTECTION:
        return True
    return await is_premium_user(user_id)

async def get_subtitle_caption(file_name, file_size):
    """Generate caption for subtitle files using custom format."""
    return SUBTITLE_CAPTION.format(file_name=file_name, file_size=get_size(file_size))

async def check_sendall_permission(user_id):
    """Check if user can use send all button (premium only)."""
    if SHOW_SENDALL_NON_PREMIUM:
        # Show button but will alert non-premium
        return True
    else:
        # Hide button for non-premium
        return await is_premium_user(user_id)

# Request System Functions
async def save_request(user_id, movie_name, year, title):
    """Save a subtitle request to database."""
    from database.users_chats_db import db
    await db.add_request(user_id, movie_name, year, title)

async def get_request_status(movie_name):
    """Check if a movie already has subtitles (for request system)."""
